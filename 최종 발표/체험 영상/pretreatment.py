...
def readCSV(num = 1):
    if num == 0:
        ...
        weather = pd.read_csv('weather2.csv', encoding='euc-kr',low_memory = False)
        stnInfo = pd.read_csv('stnInfo.csv', encoding='euc-kr')

        return weather, stnInfo
    else:
        weather = pd.read_csv('pWeather.csv', encoding='euc-kr',low_memory = False)
        return weather
...
def run(job = 1):
    if job == 0:
        ...
        pWeather.to_csv('pWeather5.csv', index=False, na_rep=0, encoding='euc-kr')
    else:
        print('---')
